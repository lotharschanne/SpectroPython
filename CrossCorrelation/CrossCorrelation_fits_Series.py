...
A cross-correlation of a series of target spectra is performed with respect to
a template spectrum is performed. Both are available as fits.
The RVs and optionally barycentrically corrected RVs are printed out and saved
in a file.
If the barycentrically corrected RVs are to be calculated, the observer and
object coordinates must be adjusted in the script. By default the star
coordinates are taken from the Internet by entering the object name
(e.g. alp Ori).
...
altitude = 200
# Here are the coordinates in the form of a list
longitude = [7, 28, 39.0]
latitude = [49, 28, 31.0]
...
print("Minimum and maximum in the template [ADU]: ", tf.min(), "  ", tf.max())
...
# Sort alphabetically. If the spectrum files are named correctly,
...
time.sleep(1.)
frage_bary = input(
    'Do you want to correct the RV barycentrically? Then enter "y": ')
...
for i in range(len(filelist)):
    ...
    plt.title("Cross-correlation function " + filelist[i], size=8)
    plt.grid(True)
    plt.xlabel("km/s")
    plt.pause(1)
    # plt.savefig(filelist[i]+'_CroCo.png', format='png)
    plt.close()

print("Spectrum           ", '  JD    ',
      ' barycentric correction ', " RV [km/s] ", "  RV_bc [km/s]")
for i in range(len(filelist)):
    print(filelist[i], obs_time[i], corr[i], RV[i], RV_bc[i])

data = Table([filelist, obs_time, corr, RV, RV_bc], names=[
             "Spectrum", "JD", 'barycentric correction', "RV", 'RV_bc'])
ascii.write(data, "CC_RV_List_" + star + ".dat", overwrite=True, format="tab")
