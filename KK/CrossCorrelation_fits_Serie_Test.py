...
Die RV's und wahlweise baryzentrisch korrigierten RV's werden ausgedruckt und
in einer Datei abgespeichert.
Falls die baryzentrische korrigierten RV's berechnet werden sollen, müssen
die Beobachter- und die Objektkoordinaten im Skript angepasst werden. Standardmäßig
werden die Sternkoordinaten aber durch Eingabe des Objektnamens (z.B. alp Ori)
...
for i in range(len(filelist)):
    ...
    rv, cc = pyasl.crosscorrRV(
        w, f, tw, tf, -70.0, 70.0, 0.1, mode="doppler", skipedge=100
    )
...
data = Table([filelist, obs_time, corr.round(2), RV.round(2), RV_bc.round(2)],
             names=["Spektrum", "JD-OBS", 'baryz. Korrektur', "RV", 'RV_bc'])
