...
fits-Format, nicht heliozentrisch korrigiert !!!). Berechnet aus dem
Beobachtungszeitpunkt und den (anzupassenden)
Koordinaten des Beobachters und Objekts die heliozentrische Korrektur,
fitted die gewählte Linie per Radial basis function (RBF)
...
for i in range(len(filelist)):
    ...
    # ************************************************************
    # Radial basis function (RBF) über das Spektrum im Intervall smooth anpassen.
    # 0. = Funktion geht durch alle Punkte, also extrem weiche Funktion
    # >0. = Ausgleich, je höher, je rigider die Funktion
    rbf = Rbf(intervallwave, intervallflux, smooth=10
    # *************************************************************
    ...
    print("RV baryzentrisch korrigiert in km/s: ", RV_bc[i])
...
# # Plot der RV's über die Zeit (JD)
# fig = plt.figure()
...
fr=input('Wenn Sie fertig mit der Betrachtung der Grafiken sind und das\
 Programm beenden möchten, drücken sie eine Taste')
plt.close('all')
