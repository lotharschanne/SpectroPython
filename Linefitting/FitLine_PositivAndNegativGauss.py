...
# -*- coding: utf-8 -*-
"""
The script is used for Gaussian modeling of an emission line profile which, as with
beta Lyrae consists of a Gaussian emission line with a central absorption contained therein.

The script loads a series of spectra normalized to the continuum in fits format
format. If the line profile in a series is Doppler shifted or completely different f
rom the first spectrum, the fitting may not work. Such spectra must then be
...
ergebnis.to_excel('Results_Gauss.xlsx', sheet_name='Results')
