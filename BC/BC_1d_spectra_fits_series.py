...
Das Skript berechnet die baryzentrische Korrektur BC und das korrigierte JD als
HJD einer Serie von 1d-Spektren im Fits-Format (xxx.fits) eines Objekts.
...
baryzentrische Korrektur BC und speichert sie als fits ab (und nach
auskommetieren der Zeilen 206 bis 213 auch als asci-Tabelle).
...
for i in range(len(filelist)):
    ...
    if frage == 'y':
        ...
        header["CRVAL1"] = wave_bc[0]
        header["CRPIX1"] = 1
        header["NAXIS1"] = len(wave_bc)
...
ascii.write(data, "BC_Liste_" + ".dat", overwrite=True, format="tab")
