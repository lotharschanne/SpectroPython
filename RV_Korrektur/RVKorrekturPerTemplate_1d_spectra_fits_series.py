...
for i in range(len(filelist)):
    ...
    rv, cc = pyasl.crosscorrRV(
        w, f, tw, tf, -100., 100., 0.5, mode='doppler', skipedge=20)
    ...
    newfile = filelist[i].rstrip(".fit") + "_RVcorrected_perTemplate.fit"
    header["RVCORR"] = (rv[maxind], "km/s, corrected")
    fits.writeto(newfile, flux_rv, header, overwrite=True,
                 output_verify="silentfix")
    print("New begin of wavelength scale, CRVAL1: ", header["CRVAL1"])
